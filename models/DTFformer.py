...
# ===================================================================================
# 核心模块定义.0.370。双分支结构，加了中间层进行信息交互
# ===================================================================================
...
class FreqBranch(nn.Module):
    def __init__(self, configs, patch_len):
        ...
        # self.last_importance_heatmap = None #画图
        # self.last_fft_amp_before = None  # 用于 "Before" 热力图，画图
        # self.last_fft_amp_after = None #画图
    ...
    def forward(self, patches):
        ...
        # if not self.training:   #画图
        #     self.last_importance_heatmap = importance[0].detach().cpu()
        #     self.last_fft_amp_before = fft_amp[0].detach().cpu()
        ...
        # fft_amp_after = torch.sqrt(fft_real_filtered ** 2 + fft_imag_filtered ** 2)#画图
        # if not self.training:   #画图
        #     self.last_fft_amp_after = fft_amp_after[0].detach().cpu()
        ...
        # 7. 返回
        #  如果想返回 importance 用于分析,生成热力图，模型的 forecast 方法也需要修改)
        return freq_feat

# ===================================================================================
# FreqEnhancedAttention 模块
# ===================================================================================
...
class GatedInteractionLayer(nn.Module):
    ...
    def forward(self, time_feat, guided_feat):
        ...
        # --- 修改点：在返回值里加上 gate_for_time ---
        # 这样 Hook 才能捕获到它，用来画图 (b) The Filter
        return next_time_feat, next_guided_feat, gate_for_time
